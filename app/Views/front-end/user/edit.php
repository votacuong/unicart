<?php if ( is_array(session()->get('messages')) && count(session()->get('messages')) > 0 ):?>
<div class="card h-100 thecup-container" style="margin-bottom:30px;">
...
<?php if (!empty($this->data['details']['phone'])):?>
	<?php
	$number = $this->data['details']['phone'];
	$country = substr($number, 1, 2);
	$number = substr($number, -(strlen($number)-3));	?>
	<script type="text/javascript">
	jQuery(document).ready(function(){
		$( "#countryCode").val('<?php echo $country;?>');
		$( "#Phone").val('<?php echo $number;?>');
	});
	</script>
<?php endif;?>
	<div class="card h-100 thecup-container">
	  <div class="card-body">
		...
		<?php VLang::__e('USER_EDITUSER_SETTING'); echo $this->data['details']['firstname'].' '.$this->data['details']['lastname'];?>
		<?php else:?>
		<?php VLang::__e('USER_NEWUSER');?>
		<?php endif;?>
		</h4>
		<form class="forms-sample" method="post" action="<?php echo v_base_url('user/edit');?>">
		  <div class="col-auto">
		  ...
		  <div class="col-auto">
			...
			<?php
			   if ( is_file(FCPATH . '../public/images/users/'.$this->data['details']['id'].'.png') ){
					$store = v_base_url('public/images/users/'.$this->data['details']['id'].'.png?time='.date("Y-m-d H:i:s"));
				}elseif ( is_file(FCPATH . '../public/images/users/'.$this->data['details']['id'].'.jpg') ){
					$store = v_base_url('public/images/users/'.$this->data['details']['id'].'.jpg?time='.date("Y-m-d H:i:s"));
				}elseif ( is_file(FCPATH . '../public/images/users/'.$this->data['details']['id'].'.jpeg') ){
					$store = v_base_url('public/images/users/'.$this->data['details']['id'].'.jpeg?time='.date("Y-m-d H:i:s"));
				}else{
					$store = v_base_url('public/images/default.jpg');
				}
				?>
				<img src="<?php echo $store;?>" width="150px" id="profile-img-user"/>
			<div class="input-group col-xs-12">
			  <input type="text" class="form-control file-upload-info" disabled="" placeholder="<?php VLang::__e('USER_EDIT_UPLOAD_PHOTO');?>">
			  <span class="input-group-append">
				<button class="file-upload-browse btn btn-primary" type="button" onCLick="jQuery('#photo').click();upload = true;"><?php VLang::__e('USER_EDIT_UPLOAD_PHOTO');?></button>
			  </span>
			</div>
		  </div>
		  <div class="col-auto">
...
function uploadImage()
{
	...
	$.ajax({
		   url : '<?php echo v_base_url('user/uploadimage');?>',
		   type : 'POST',
		   data : formData,
		   processData: false,
		   contentType: false,
		   success : function(data) {
			   ...
			   jQuery('#profile-img-user').attr('src', data.user);
			   jQuery('#imagePath').val(data.link);
			   upload = false;
			   jQuery('#photo').val('');
		   }
	});
